...
@Component({
  ...
  styleUrl: './registermedecin.component.css'
})
export class RegistermedecinComponent implements OnInit {
  specialiteOptions =["Cardiologie","Dermatologie","Gastroentérologie","Neurologie","Oncologie","Orthopédie","Psychiatrie","Radiologie","Gynécologie"]
  ...
  onSubmit(): void {
    ...
    this.authService.getAlluesrListuser().subscribe((data) => {
      ...
      if (!existsInBdphone) {
        ...
        this.authService.register_medecin(user, email,specialite, lang).subscribe(
          (data) => {
            console.log(data);
            // console.log(data);
